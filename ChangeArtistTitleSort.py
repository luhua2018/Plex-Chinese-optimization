...
def updateSortTitle(rating, item):
    ...
    data = fetchPlexApi("/library/sections/"+sectionNum+"/all?type=8&id=%s&titleSort.value=%s&" %
    ...
    while toDo:
        if len(sectionNum):
            url = "/library/sections/" + sectionNum + \
                "/all?type=8&X-Plex-Container-Start=%i&X-Plex-Container-Size=%i" % (
                    start, size)
...
def ChangeArtistTitleSort(URL, TOKEN):
    global PLEX_URL, PLEX_TOKEN, sectionNum
    PLEX_URL = URL
    PLEX_TOKEN = TOKEN
