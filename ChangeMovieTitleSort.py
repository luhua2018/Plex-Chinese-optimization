...
def updateSortTitle(rating, item):
    ...
    data = fetchPlexApi("/library/sections/"+sectionNum+"/all?type=1&id=%s&titleSort.value=%s&" %
    ...
    while toDo:
        if len(sectionNum):
            ...
            for movie in elements:
                ...
                if ('titleSort' in movie) and (check_contain_chinese(movie["titleSort"]) == False):
                    continue

                #########################################################
                # 此处逻辑原本的代码如下，但那应该是错误的，留下来供以后参阅
                #
                # # 判断是否已经有"排序标题(TitleSort)"
                # if 'titleSort' in movie:
                #     con = movie["titleSort"]
                #     # 存在 TitleSort 且包含中文字符的话，则修改
...
def ChangeMovieTitleSort(URL, TOKEN):
    global PLEX_URL, PLEX_TOKEN, sectionNum
    PLEX_URL = URL
    PLEX_TOKEN = TOKEN
    ...
    for section in plex.library.sections():
        ...
        if section.type == 'movie' and (section.language == 'zh-CN' or section.language == 'xn'):
            print("准备处理 电影　 TitleSort，库：", section.key, section.type,
