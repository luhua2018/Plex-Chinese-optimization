...
def updateSortTitle(rating, item):
    ...
    data = fetchPlexApi("/library/sections/"+sectionNum+"/all?type=2&id=%s&titleSort.value=%s&" %
    ...
    while toDo:
        if len(sectionNum):
            ...
            for show in elements:
                ...
                if ('titleSort' in show) and (check_contain_chinese(show["titleSort"]) == False):
                    continue
                # 创建处理后的 TitleSort
                key = show["ratingKey"]
                title = show["title"]
...
def ChangeShowTitleSort(URL, TOKEN):
    global PLEX_URL, PLEX_TOKEN, sectionNum
    PLEX_URL = URL
    PLEX_TOKEN = TOKEN
    ...
    for section in plex.library.sections():
        ...
        if section.type == 'show' and (section.language == 'zh-CN' or section.language == 'xn'):
            print("准备处理 剧集　 TitleSort，库：", section.key, section.type,
